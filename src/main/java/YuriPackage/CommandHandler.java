...
public class CommandHandler {
  ...
  public SendMessage getOutMessforYuri() {
    return outMessforYuri;
  }

  private SendMessage outMessforYuri = null;
 ...
 void handleFirstTextOfCommand(String command, Long chatID)
    ...
    if (command.equals("/start")) {
      ...
      outMess.setText(result_prod_and_prices + "Введите /find, чтобы найти какой-то конкретный товар");

      this.outMessforYuri = new SendMessage();
      creator.createKeyboardCategoriesToUser(chatID);
      outMessforYuri = creator.getSm();
      outMessforYuri.setChatId(chatID.toString());
      currentCommand.put(true, "Default command");//ставим дефолтную команду
    }
